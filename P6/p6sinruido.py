...
plt.title('Visualizacion de los primeros 5 bloques de bits modulados sin ruido')
plt.ylabel('Amplitud')
plt.xlabel('tiempo(s)')
plt.savefig('senalsinruido.png')
plt.close()
...
#analytic_signal = hilbert(senal, N=None, axis= 1)
#amplitude_envelope = np.abs(analytic_signal)
#plt.plot(t[0:pb*pts],amplitude_envelope[0:pb*pts])
#plt.title('Visualizacion de los primeros 5 bloques de bits modulados sin ruido')
#plt.ylabel('Amplitud')
#plt.xlabel('tiempo(s)')
#plt.savefig('senalsinruido2.png')
#plt.close()
...
count = 0
valores = []
#high_i, low_i = envelopes_idx(senal[0:len(bits)*pts], 1, 1) 
for i in range(len(bits)):
    ...
    #print("symb:", symb)
    #print("high_i: ", high_i, "low_i: ", low_i)
    #print("lens")
    #print(len(high_i))  
    if(symb == 1):
        l = 0
...
for valor in valores:
    if(valor < -80):
        zz = [0,0]
        des.append(zz)
    elif(valor > -79 and valor < -50):
        zo = [0,1]
        des.append(zo)
    elif(valor > 50 and valor < 79):
        oz = [1,0]
        des.append(oz)
    elif(valor > 80):
        oo = [1,1]
